...
function isBinarySearchTree(treeRoot) {
  ...
  if (treeRoot.left) {
    nodes.push([treeRoot.left, 'left']);
  }

  if (treeRoot.right) {
    nodes.push([treeRoot.right, 'right']);
  }

  // depth first walk
  while (nodes.length) {
    const [node, side] = nodes.pop();
    if (side === 'left' && node.value > treeRoot.value) {
      return false;
    }

    if (side === 'right' && node.value < treeRoot.value) {
      return false;
    }
